      ...
      <div className="relative group/item">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            {hasSubItems ? (
              <Layers size={depth === 0 ? 12 : 10} className={cn("transition-colors", isActive ? "text-primary" : "text-muted-foreground/40")} />
            ) : (
              <div className={cn(
                "w-1 h-1 rounded-full transition-all",
                isDirectlyActive ? "bg-primary shadow-[0_0_5px_var(--color-primary)]" : "bg-white/10 group-hover/link:bg-white/30"
              )} />
            )}
            <span className={cn(
              "transition-colors",
          ...
          <div className="flex items-center gap-1">
            {hasSubItems ? (
              ...
              >
                {isExpanded ? <ChevronDown size={depth === 0 ? 10 : 8} /> : <ChevronRight size={depth === 0 ? 10 : 8} />}
              </button>
            ) : (
              <ChevronRight 
                size={depth === 0 ? 12 : 10} 
                className={cn(
                  ...
                  isDirectlyActive ? "opacity-100 translate-x-0 text-primary" : "opacity-0 -translate-x-2 group-hover/item:opacity-40 group-hover/item:translate-x-0"
                )} 
              />
            )}
          </div>
            ...
            <div key={idx} className="flex flex-col gap-3">
              ...
              {!isSectionCollapsed && (
                <div className="flex flex-col gap-1 ml-3.5 pl-4 border-l border-white/[0.05] relative">
                  {section.items.some((item: any) => findActive(item, pathname)) && (
...
const findActive = (item: any, pathname: string): boolean => {
  ...
  if (item.items) return item.items.some((child: any) => findActive(child, pathname));
  return false;
};
