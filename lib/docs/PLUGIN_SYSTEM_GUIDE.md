...
// Plugin with lifecycle and routes
{
  ...
  registerRoutes: (app) => {
    app.get('/db/health', (req, res) => res.xJson({ connected: db.isConnected() }));
  }
}
```
      ...
      () => ({
        ...
        registerRoutes: (app) => {
          app.get("/hello", (req, res) => {
            res.xJson({ message: "Hello from plugin!" });
          });
        },
      }),
    ],
  ...
  return {
    ...
    registerRoutes: (app) => {
      app.get("/db/health", (req, res) => {
        res.xJson({ connected: db.isConnected() });
      });
    },
  };
}
  ...
  return {
    ...
    registerRoutes: (app) => {
      app.post("/auth/login", async (req, res) => {
        ...
        const token = await generateToken(username, password, config.secret);
        res.xJson({ token });
      });
