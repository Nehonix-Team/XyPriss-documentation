...
export default function RequestSignaturesPage() {
  return (
    <div className="prose prose-invert max-w-none">
      ...
      <p>
        XyPriss provides{" "}
        <strong>a powerful request signature authentication system</strong> that
        allows you to secure your APIs using cryptographic signatures. This
        method ensures that only authorized clients can access your API,
        requests cannot be tampered with in transit, and replay attacks are
        prevented.
      </p>

      <SectionHeading level={2}>Quick Start</SectionHeading>
      <p>
        Enable request signature validation globally by providing a shared
        secret in your security configuration:
      </p>
      <CodeBlock
        language="typescript"
...
const server = createServer({
    security: {
      ...
      <p>
        Clients must include a cryptographic signature in the{" "}
        <code>XP-Request-Sig</code> header and a timestamp in the{" "}
        <code>X-Timestamp</code> header. The signature is an HMAC-SHA256 hash of
        the request method, URL, body, and timestamp.
      </p>

      <Steps>
        <Step title="Prepare Data String">
          Concatenate the components:{" "}
          <code>METHOD + URL + BODY + TIMESTAMP</code>.
        </Step>
      ...
      <CodeBlock
        language="typescript"
        title="XSec-M Client Example"
        code={`import { Hash } from "xypriss-security";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
        <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl">
          ...
          <p className="text-[10px] text-slate-400 m-0">
            Always use cryptographically secure random strings for your shared
            secrets. Never use weak passwords.
          </p>
        </div>
        <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl">
          ...
          <p className="text-[10px] text-slate-400 m-0">
            Signatures prevent tampering but not interception. Always use HTTPS
            in production to encrypt the headers.
          </p>
        </div>
      </div>

      <Callout type="danger" title="Clock Synchronization">
        Ensure your server and client clocks are synchronized. Requests will
        fail with a "Signature expired" error if the time difference exceeds{" "}
        <code>maxAge + clockSkew</code>.
      </Callout>
