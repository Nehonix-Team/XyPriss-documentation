  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 mb-8">
        ...
        <SectionHeading level={2} id="filtered-listing">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
        <div className="p-5 rounded-xl bg-white/[0.02] border border-white/5">
          ...
          <CodeBlock
            language="typescript"
            code={`const tsFiles = __sys__.fs.lsRecursive("ROOT://src", f => f.endsWith(".ts"));`}
          />
        </div>
        <div className="space-y-2">
          {[
            ...
            {
              n: ".lsFullPath(p)",
              d: "Returns fully resolved absolute paths.",
            },
          ].map((u, i) => (
            <div
              key={i}
              className="p-3 rounded-lg bg-white/[0.01] border border-white/5 flex justify-between items-center"
            >
      ...
      <SectionHeading level={2} id="json-handling">
        JSON Automation
      </SectionHeading>
      <p className="mb-4 text-muted-foreground">
        Native JSON serialization using{" "}
        <code className="text-primary">XStringify</code> with safe fallback
        options.
      </p>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
        <div className="p-5 rounded-xl bg-white/[0.02] border border-white/5 border-l-primary/30">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            Returns <code className="text-primary">default</code> instead of
            throwing if the file is missing or malformed.
          </p>
        </div>
        <div className="space-y-2">
          <div className="p-3 rounded-lg bg-white/[0.01] border border-white/5 font-mono text-xs text-white">
            .readJson(path)
          </div>
          <div className="p-3 rounded-lg bg-white/[0.01] border border-white/5 font-mono text-xs text-white">
            .writeJson(path, data)
      ...
      <SectionHeading level={2} id="binary-io">
        Binary & Raw I/O
      </SectionHeading>
        ...
        <div className="p-5 rounded-xl bg-white/[0.02] border border-white/5 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Bypass text encoding entirely. Works with raw Node{" "}
              <code className="text-primary">Buffer</code> objects.
            </p>
          </div>
        </div>
        <div className="p-5 rounded-xl bg-white/[0.02] border border-white/5 flex items-start gap-4">
      ...
      <SectionHeading level={2} id="stats-utilities">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 my-6">
        {[
          ...
          <div
            key={i}
            className="p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:border-primary/20 transition-colors group"
          >
            <u.i
              size={20}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-6">
        {[
          ...
          {
            n: ".writeIfNotExists(path, data)",
            d: "Conditional write operation.",
          },
