    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 mb-8">
        ...
        <p className="leading-relaxed">
          These methods descend directly from the{" "}
          <code className="text-primary">FSCore</code> instance and interact
          natively with the underlying XHSC engine.
        </p>
      </div>

      <SectionHeading level={2} id="ls">
        ls
      </SectionHeading>
...
const detailed = __sys__.fs.ls("/var/log/app", { stats: true });`}
      ...
      <SectionHeading level={2} id="read">
        read / readSync
      </SectionHeading>
      <p className="text-muted-foreground mb-4">
...
const template = __sys__.fs.readSync("ROOT://index.html");`}
      ...
      <SectionHeading level={2} id="streams">
        Streams
      </SectionHeading>
      <p className="text-muted-foreground mb-4">
...
stream.pipe(res);`}
      ...
      <SectionHeading level={2} id="write">
      ...
      <SectionHeading level={2} id="move-copy">
        Copy & Move
      </SectionHeading>
      ...
      <SectionHeading level={2} id="rm-mkdir">
        Deletion & Structure
      </SectionHeading>
      <div className="space-y-4 my-6">
        <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5">
          ...
          <p className="text-sm text-muted-foreground">
            Deletes a file or directory. Use{" "}
            <code className="text-primary">force: true</code> for recursive
            deletion.
          </p>
        </div>
        <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5">
          ...
          <p className="text-sm text-muted-foreground">
            Creates directories.{" "}
            <code className="text-primary">parents: true</code> enables
            recursive creation (mkdir -p).
          </p>
        </div>
      </div>

      <SectionHeading level={2} id="open-close">
        Stateful Handles (open/close)
      </SectionHeading>
      <p className="leading-relaxed mb-6">
        The <code className="text-primary">open</code> method provides a
        stateful toolbox for efficient multi-operation tasks on a single handle.
      </p>
        ...
        <table className="w-full text-left border-collapse border border-white/5">
          <thead>
            <tr className="bg-white/5">
              <th className="p-3 border border-white/5 text-sm font-bold uppercase tracking-wider">
                Flag
              </th>
              <th className="p-3 border border-white/5 text-sm font-bold uppercase tracking-wider">
                Mode
            ...
            {[
              {
                f: "'r'",
                m: "Read",
                d: "Open for reading (default). Fails if missing.",
              },
              {
                f: "'r+'",
                m: "R/W",
                d: "Open for reading and writing. Fails if missing.",
              },
              {
                f: "'w'",
                m: "Write",
                d: "Open for writing. Created if missing, truncated if exists.",
              },
              {
                f: "'a'",
                m: "Append",
                d: "Open for appending. Created if missing.",
              },
            ].map((row, i) => (
              <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                <td className="p-3 border border-white/5 font-mono text-primary text-xs">
                  {row.f}
                </td>
                <td className="p-3 border border-white/5 text-xs text-white">
                  {row.m}
                </td>
                <td className="p-3 border border-white/5 text-xs text-muted-foreground">
                  {row.d}
                </td>
              </tr>
            ))}
      ...
      <Callout type="tip" title="FileHandle Toolbox">
        Using the callback pattern in <code className="text-primary">open</code>{" "}
        gives you access to a toolbox with{" "}
        <code className="text-primary">read()</code>,{" "}
        <code className="text-primary">write()</code>, and{" "}
        <code className="text-primary">seek()</code> methods that communicate
        via optimized IPC sockets.
      </Callout>
    ...
    await file.write(" [EOF SIGNATURE]");
      ...
      <SectionHeading level={2} id="more-core">
        Utility Primitives
      </SectionHeading>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-6">
        {[
          ...
          {
            n: ".chmod(path, mode)",
            d: "Change Unix permissions (e.g., '755').",
          },
        ].map((u, i) => (
          <div
            key={i}
