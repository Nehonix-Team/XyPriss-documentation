...
export default function FsSearchPage() {
  return (
    <div className="flex flex-col gap-6">
      ...
      <SectionHeading level={2} id="full-text">
        Full-Text Search
      </SectionHeading>
      <p className="mb-4">
        Query the contents of entire directory trees using highly optimized
        regex patterns.
      </p>
      <div className="space-y-4 my-6">
        <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/5">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Returns an array of{" "}
            <code className="text-primary">SearchMatch</code> objects including
            filename, line number, and content snippet.
          </p>
          <CodeBlock
            language="typescript"
...
results.forEach(m => console.log(\`\${m.file}:\${m.line} -> \${m.content}\`));`}
      ...
      <SectionHeading level={2} id="discovery">
        File Discovery
      </SectionHeading>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
        <div className="p-5 rounded-xl bg-white/[0.02] border border-white/5">
          ...
          <CodeBlock
            language="typescript"
            code={`const ts = __sys__.fs.findByPattern("./src", "*.ts");`}
          />
        </div>
        <div className="p-5 rounded-xl bg-white/[0.02] border border-white/5">
          <div className="flex items-center gap-3 mb-3 text-primary font-bold uppercase tracking-widest text-[10px]">
          ...
          <CodeBlock
            language="typescript"
            code={`const pngs = __sys__.fs.findByExt("./assets", "png");`}
          />
        </div>
      </div>
      ...
      <div className="space-y-6 my-6">
        <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/5">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Mass rename files matching a regex. Use{" "}
            <code className="text-primary">dryRun: true</code> to preview
            changes.
          </p>
          <CodeBlock
            language="typescript"
